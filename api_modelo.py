...
#Cria as tabelas no banco (em produção usar o Alembic)
Base.metadata.create_all(engine)

model = joblib.load('modelo_iris.pkl')
logger.info('Nodelo carregado com sucesso')

app = Flask(__name__)
predictions_cache = {}

TEST_USERNAME = 'admin'
TEST_PASSWORD = 'segredo'
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        #pegar token do header Authorozation: Bearer <token>
...
def list_predictions():
    ...
    db = SessionLocal()
    preds = db.query(Prediciton).order_by(Prediciton.id.desc()).limit(limit).offset(offset).all()
    db.close()
    results = []
    for p in preds:
        results.append({
